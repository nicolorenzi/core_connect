...
if not app.secret_key:
    raise RuntimeError(
        "SECRET_KEY environment variable is required for secure sessions")

DEFAULT_EMAILS = [e.strip() for e in os.getenv(
    "DEFAULT_EMAILS", "").split(",") if e.strip()]
...
if db_password and db_url.startswith("postgresql"):
    ...
    DATABASE_URL_OBJECT = _normalize_database_url(db_url)
...
def ensure_workout_user_column():
    ...
    existing_columns = {col["name"]
                        for col in inspector.get_columns("Workouts")}
...
def ensure_exercise_columns():
    ...
    inspector = inspect(engine)
    existing_columns = {col["name"]
                        for col in inspector.get_columns("Exercises")}

    with engine.begin() as conn:
        if "intensity" in existing_columns:
            conn.execute(
                text('ALTER TABLE "Exercises" RENAME TO "Exercises_old"'))
            id_column_definition = _primary_key_column_definition()
            conn.execute(text(
        ...
        if "set_label" not in existing_columns:
            conn.execute(
                text('ALTER TABLE "Exercises" ADD COLUMN set_label VARCHAR DEFAULT \'1\''))
        if "stored_exercise_id" not in existing_columns:
            conn.execute(
                text('ALTER TABLE "Exercises" ADD COLUMN stored_exercise_id INTEGER'))


def seed_default_users():
    if not DEFAULT_EMAILS:
        logger.debug(
            "Skipping default user seed because no DEFAULT_EMAILS configured.")
        return
    if not DEFAULT_PASSWORD:
        logger.warning(
            "DEFAULT_PASSWORD is missing; skipping default user seed.")
        return
    if not is_strong_password(DEFAULT_PASSWORD):
        logger.warning(
            "DEFAULT_PASSWORD does not meet the configured password policy; skipping default user seed.")
        return
    ...
    for email in DEFAULT_EMAILS:
        if email not in existing_users:
            new_users.append(
                User(email=email, password=generate_password_hash(DEFAULT_PASSWORD)))
...
def normalize_exercise_name(value):
    if not value:
        return ""
    normalized_words = [word.capitalize()
                        for word in value.strip().split() if word]
...
def workout():
    ...
    if request.method == "POST":
        ...
        if not stored_exercise:
            stored_exercise = StoredExercise(
        ...
        workout_entry = db.query(Workout).filter_by(
            date=date, user_id=user_id).first()
